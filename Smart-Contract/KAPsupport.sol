...
// File contracts/shared/libraries/Create2.sol
...
// File contracts/shared/token/CROP20.sol

pragma solidity ^0.8.0;

contract CROP20 is KAP20, IKAP20AdminApprove {
  ...
  constructor(
    ...
    uint256 _acceptedKycLevel,
    address _transferRouter
  ) KAP20(_name, _symbol, _decimals, _adminRouter, _committee, _kyc, _acceptedKycLevel, _transferRouter) {
    tokenBuilder = _tokenBuilder;
    totalSupply = _totalSupply;
    _balances[_tokenBuilder] = _totalSupply;
    emit Transfer(address(0), _tokenBuilder, totalSupply);
  }
  ...
  function mint(address _to, uint256 _amount) external whenNotPaused onlyAdmin returns (bool) {
    _mint(_to, _amount);
    return true;
  }
...
// File contracts/TokenBuilder.sol

pragma solidity ^0.8.0;

contract TokenBuilder is Authorization {
  event CreateToken(address indexed _creator, address indexed _tokenAddress);
  ...
  function createToken(
    ...
    bytes memory bytecode = _bytecode(_name, _symbol, _decimals, _totalSupply);
    address tokenAddress = _deploy(bytecode);

    emit CreateToken(msg.sender, tokenAddress);
  }
  ...
  function _deploy(bytes memory bytecode_) internal returns (address) {
    bytes32 _salt = keccak256(abi.encodePacked(address(this)));
    address tokenAddress = Create2.deploy(0, _salt, bytecode_);
    return tokenAddress;
  }
}
